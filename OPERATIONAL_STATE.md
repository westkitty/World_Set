...
<!-- operational-state:metadata
{
  ...
  "state_revision": 16,
  "last_updated": "2026-09-18T02:06:23Z",
  "current_baseline": {
    "identity": "published WS-GAME-07 at 3cc725579d6599a3bd31a8fa5f7ccf2b8bfb1ab5: ambient events are profile-gated, Mars has a bounded transient dust-devil field slice, radar/scanner/ExpeditionRecord surface environmental phenomena, and EnvironmentalAudioDirector consumes every world audio.environment plus spatialReactivity through the existing ambience bus; all five project suites plus git diff --check pass; direct heard/visible runtime proof remains unverified",
    "state": "partially-verified",
    "last_verified": "2026-09-18T02:06:23Z"
  },
...
- WS-GAME-06 is published at `679735b48d94d848eed1a1e272d432448e2a6036`: `VisibilityModel` consumes world `baseRange` and `conditionSensitivity`, applies condition occlusion as a bounded eased multiplier over each world authored fog baseline, and exposes effective visibility range to the environmental scanner.
- Direct visible fog/readability behavior remains unverified in the current automation environment, so WS-GAME-06 remains source/integration verified rather than promoted to visible-runtime verified.
- WS-GAME-07 is published at `3cc725579d6599a3bd31a8fa5f7ccf2b8bfb1ab5`: `AmbientEventDirector` gates transient events through each world profile `events` list, Mars implements a bounded dust-devil field slice, radar/scanner expose nearby phenomena, phenomenon scans persist to `ExpeditionRecord`, and `EnvironmentalAudioDirector` consumes `audio.environment` plus `spatialReactivity` through the existing ambience mixer bus.
- Direct auditory mix quality and visible Mars dust-devil behavior remain unverified in the current automation environment, so WS-GAME-07 remains source/integration verified rather than promoted to heard/visible-runtime verified.
...
- Traversal profile values have runtime consumers through WS-GAME-03. Instrument profile values have runtime consumers through WS-GAME-04. Visibility profile values have runtime consumers through WS-GAME-06. Audio `environment` and `spatialReactivity` values plus event eligibility now have runtime consumers through WS-GAME-07.
...
| INV-016 | Visibility profiles and live conditions drive bounded fog/readability response | partially-verified | INV-16 PASS; `VisibilityModel` consumes `baseRange` and `conditionSensitivity`, preserves authored fog baselines, eases density changes, updates after `WorldConditionDirector`, and reports effective scanner visibility | direct browser condition-transition and readability proof |
| INV-017 | Ambient events and environmental audio are profile-driven and bounded | partially-verified | INV-17 PASS; Mars dust devil is profile-gated and bounded, condition-responsive, visible to radar/scanner, persisted to ExpeditionRecord; all declared audio environments map to spectral identities, use the ambience bus, keep a capped calm bed, and scale with `spatialReactivity`/condition/proximity | direct audible mix proof plus visible Mars event/radar/scanner journey |
## 12. Current Change Scope and Impact Radius
- Published WS-GAME-07 feature commit `3cc725579d6599a3bd31a8fa5f7ccf2b8bfb1ab5` changed `index.html` and `tools/test_regression.py`; this operational-state closure records publication. It adds profile-gated transient ambient events, the Mars dust-devil field slice, radar/scanner/field-record phenomenon integration, and a bounded spectral environmental-audio layer without changing dependencies, authored assets, parcel counts, packaging, collision authority, or the existing mixer controls.
- Published WS-GAME-06 feature commit `679735b48d94d848eed1a1e272d432448e2a6036` changed `index.html` and `tools/test_regression.py`; this operational-state closure records publication. It adds centralized condition-driven visibility and scanner visibility reporting without changing dependencies, world geometry, assets, parcel counts, packaging, collision, or authored fog colors/baselines.
...
- Revision 15 — 2026-09-18: published WS-GAME-06 to `origin/main` at `679735b48d94d848eed1a1e272d432448e2a6036` after all five project suites and `git diff --check` passed. Added centralized condition-driven visibility that preserves authored fog baselines, consumes `baseRange` and `conditionSensitivity`, eases fog-density changes, and exposes effective visibility in scanner output. Local feature SHA and remote `refs/heads/main` matched after push; direct visual fog/readability proof remains pending.
- Revision 16 — 2026-09-18: published WS-GAME-07 to `origin/main` at `3cc725579d6599a3bd31a8fa5f7ccf2b8bfb1ab5` after all five project suites and `git diff --check` passed. Preserved and completed the existing local ambient-event packet by gating events through profile data, making all declared audio environments select spectral identities, retaining a bounded calm-world bed, and routing condition/proximity response through the existing ambience bus. Local feature SHA and remote `refs/heads/main` matched after push; direct audible/visible runtime proof remains pending.
